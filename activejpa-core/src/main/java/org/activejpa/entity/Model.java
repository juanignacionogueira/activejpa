...
/**
 * <p>
 * Base class for all entities. Embeds entity manager in it and provides a bunch
 * of DAL abstractions to make data access a lot simpler. The static methods are
 * to be implemented by the entity classes and will be done at runtime through
 * activejpa instrumentation.
 * 
 * <p>
 * This allows activerecord style of usage,
 * 
 * <pre>
 * Person.findById(1L); Person.where("firstName", "Ganesh", "lastName",
 * "Subramanian"); Person.count(); Person.collection("accounts")
 * 
 * @author ganeshs
 *
 */
public abstract class Model extends BaseObject {
	...
	/**
	 * Returns a list of entities matching the given key value pairs. The key
	 * value pairs are supplied as arguments like (key1, value1, key2, value2)
	 * 
	 * @param paramValues
	 * @return
	 */
	...
	/**
	 * Returns a single row matching the given key value pairs. The key value
	 * pairs are supplied as arguments like (key1, value1, key2, value2)
	 * 
	 * <p>
	 * If more than one row matches or no row matches, throws an exception
	 * 
	 * @param paramValues
	 * @return
	...
	/**
	 * Returns the first row matching the given key value pairs. The key value
	 * pairs are supplied as arguments like (key1, value1, key2, value2)
	 * 
	 * @param paramValues
	 * @return
	...
	/**
	 * Starts the transaction if its not active already. Returns back the new
	 * transaction or existing active one.
	 * 
	 * @return
	 */
	...
	public <T extends Model> EntityCollection<T> collection(String name) {
		ManagedType<? extends Model> type = getEntityManager().getMetamodel().managedType(this.getClass());
